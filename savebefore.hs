...
main = do
    ...
    xmproc <- spawnPipe ("xmobar -x $HOME/.config/xmobar/xmobarrc" ++ colorScheme ++ "-xmobarrc")
    -- The old main was just this line
    -- xmonad $ ewmhFullscreen $ ewmh $ xmobarProp $ defaults
    xmonad $ ewmh $ docks $ defaults
    
--main = do 
    -- Launch xmobar
    -- xmproc <- spawnPipe "$HOME/.config/xmobar/config.hs"
    --xmproc <- spawnPipe myBarPath
    -- Launch xmonad with settings 
...
defaults = def {
      ...
      -- hooks, layouts
        ...
        logHook            = dynamicLogWithPP $ xmproc $ xmobarPP
            { ppOutput = \x -> hPutStrLn xmobarProp x
